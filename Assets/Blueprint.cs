...
";
    ...
    void Start()
    {
        ...
        previewBlockPrefab.transform.localScale = gridSize;
    ...
    void Update()
    {
        ...
        if (isPrinting)
        {
            ...
            for (int i = 0; i < previewBlocks.Length; i++)
            {
                if (i > size.magnitude)
                {
                    if (previewBlocks[i] != null) Destroy(previewBlocks[i]);
                }
                else
                {
                    if (previewBlocks[i] == null) previewBlocks[i] = Instantiate(previewBlockPrefab);
                    previewBlocks[i].transform.position = dragStartPos + size.normalized * i;
                }
            }
        }
    }
