...
func put(path: String, requestBody: String):
	var err = 0
	var http = HTTPClient.new() # Create the Client.
	err = http.connect_to_host("localhost", 8000) # Connect to host/port.
	assert(err == OK, "Failed to establish connection") # Make sure connection is OK.
	# Wait until resolved and connected.
	while http.get_status() == HTTPClient.STATUS_CONNECTING or http.get_status() == HTTPClient.STATUS_RESOLVING:
		http.poll()
		#print("Connecting...")
	...
	var request_headers: PackedStringArray = [
		...
		"Content-Length: " + str(requestBody.length())
	]


	err = http.request(HTTPClient.METHOD_PUT, path, request_headers, requestBody)
	assert(err == OK, "Failed to PUT") # Make sure all is OK.
